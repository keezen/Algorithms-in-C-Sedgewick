...
** insert, and delete the maximum, using tournaments (see B5.19, it will provide you
...
void pqueue_insert(item_t item)
{
	...
	if(root==NULL){
		copy_item(&r->item,&new->item);
		root=r;
		
		return;
	}

	if(compare_item(r->left->item,r->right->item)>0){
		copy_item(&r->item,&r->left->item);
	}else{
		copy_item(&r->item,&r->right->item);
	}
