/*
** This program will generate N 32-bit integers of a specific format (see in
** the comment of rand_format). Then sort the file using the modified binary 
** quicksort that will ignore equal leading bits.
*/
#include<stdio.h>
#include<stdlib.h>
#include<time.h>
...
/*
** This modified binary_quicksort will identify the number of leading bit positions
** where all keys are equal, in one pass, then calls a do_binary_quicksort to ignore 
** those bit positions. w is the first byte where not all keys are equal.
*/
void binary_quicksort(item_t *array,int left,int right)
{
	int w;
	...
	do_binary_quicksort(array,left,right,w);
}
